...
(* Given a list of pieces, return a list of resources that are generated from the
   pieces given, when r is rolled *)
let resources_from pcs r hexl : resource list =
	let hexs = List.map ( fun pc -> List.nth hexl pc ) pcs in
	
	List.fold_left ( fun resrc_acc (t,n) -> 
		match resource_of_terrain t with
		| Some(resrc) when n = r -> resrc :: resrc_acc
		| _ -> resrc_acc ) 
		[] hexs
